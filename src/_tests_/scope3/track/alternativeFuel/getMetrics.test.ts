    ...
    [comapnyDbAlias?.PEP]: {
        models: {
            LaneAlternateFuelType: {
                findAll: jest.fn<any>().mockResolvedValue([{ total_emissions: "asjdasd", total_shipments: 222 }]),
                count: jest.fn<any>().mockResolvedValue([{ total_emissions: "asjdasd" }])
...
const payload = {
    ...
    describeName: "Alternate fuel list of Metrics ",
    controller: AlternateFuelTypeController,
    moduleName: 'getMetrics',
    testCases: [
        {
            status: true,
            mockConnection: mockConnection,
            testName: "should return a 200 status code and data if the query is successful",
            body: { year: "2022", month: "", carrier_scac: "" },
            responseStatus: 200,
            responseMessage: "Metrics fetched successfully.",
        },
        {
            status: true,
            mockConnection: {
